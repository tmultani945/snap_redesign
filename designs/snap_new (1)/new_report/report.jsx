/* === Redesigned Run Report — Summary tab with analysis + map === */
...
const RunReport = () => {
  const baseRun = window.REPORT_RUN;
  const [t, setTweak] = useTweaks(TWEAK_DEFAULTS);

  // Recompute run based on outcome tweak
  const run = uM(() => {
    if (t.runOutcome === "ok") return baseRun;
    if (t.runOutcome === "fail") {
      const r = JSON.parse(JSON.stringify(baseRun));
      r.status = "FAILED";
      r.passCount = 0; r.failCount = 3; r.total = 3;
      r.iterations = r.iterations.slice(0,3).map((it,i) => ({
        ...it, status: "FAILED", setup: null, call: null, end: "timeout",
        error: { code: 400, msg: "Did not reach DIALING within 15 000 ms",
                 detail: "External device returned no callstate" }
      }));
      r.errors = [
        { cat: "Setup timeout", count: 3 },
        { cat: "Dropped",       count: 0 },
    ...
    // mixed
    return baseRun;
  }, [t.runOutcome, baseRun]);

  const [tab, setTab]   = pS("summary");
  const [iter, setIter] = pS(6);

  return (
    <div className="report-shell">
      <ReportContext run={run} tab={tab} setTab={setTab}/>
      {tab === "summary"  && <SummaryTab run={run} t={t} setTweak={setTweak}/>}
      {tab === "kpi"      && <KpiAnalysisTab run={run}/>}
      {tab === "session"  && <SessionTab run={run}/>}
      {tab === "artifact" && <ArtifactStub run={run}/>}

      <TweaksPanel title="Tweaks">
        <TweakSection title="Run">
          <TweakRadio label="Outcome" value={t.runOutcome} options={["ok","fail"]}
                      onChange={v => setTweak("runOutcome", v)}/>
        </TweakSection>
        <TweakSection title="Layout">
          <TweakToggle label="KPI deep-dive charts" value={t.showKpiCharts} onChange={v=>setTweak("showKpiCharts", v)}/>
          <TweakToggle label="Call setup waterfall" value={t.showWaterfall} onChange={v=>setTweak("showWaterfall", v)}/>
          <TweakToggle label="Dense matrix"         value={t.denseMatrix}   onChange={v=>setTweak("denseMatrix", v)}/>
        </TweakSection>
        <TweakSection title="Map">
          <TweakSelect label="Color by" value={t.mapColor}
                       options={[{value:"rsrp",label:"RSRP signal"},{value:"rat",label:"RAT"},{value:"errors",label:"Errors"}]}
                       onChange={v=>setTweak("mapColor", v)}/>
        </TweakSection>
      </TweaksPanel>
    </div>
  );
...
/* === HEADER === */
const ReportContext = ({ run, tab, setTab }) => {
  const tabs = [
    ["summary",  "Summary"],
    ["session",  "Session"],
    ["artifact", "Artifacts"],
  ...
  return (
    <div className="rep-context">
      <div className="rep-ctx-top">
        <button className="rep-back"><Icon name="chevronLeft" size={14}/> Runs</button>
...
const KpiDeepDive = ({ run }) => {
  ...
  const ok = run.iterations.filter(i=>i.status==="PASS" || i.rsrp != null);
...
/* === SUMMARY TAB === */
const SummaryTab = ({ run, t, setTweak }) => {
  const onJump = (i) => { /* hook to session tab in real app */ };
  return (
...
/* === Stubs for other tabs (just to show context) === */
...
const SessionStub = ({ run, iter, setIter }) => (
  <div className="rep-body">
    <div className="card"><div className="card-head"><h3>Session view</h3></div>
      <div className="card-body dim">Per-iteration drill-down · click an iteration in the matrix to open here.</div>
    </div>
  </div>
);
const ArtifactStub = () => (
